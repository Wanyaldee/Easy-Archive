...
マイルストーン1〜3完了。マイルストーン4（主要ファイルマネージャーの右クリックメニュー統合）とマイルストーン5（`.deb`パッケージング）は、いずれも実装完了、実機検証待ち。文字コード自動判定付きのCLI（`list`/`compress`/`extract`/`auto`）と、ドラッグ&ドロップで解凍・圧縮できるGUI、主要ファイルマネージャーへの右クリックメニュー連携、`.deb`パッケージのビルド一式を実装済み。詳細・今後のマイルストーンは [`docs/spec.md`](./docs/spec.md) を参照。
