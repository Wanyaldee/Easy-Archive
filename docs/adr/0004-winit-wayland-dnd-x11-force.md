...
- WSLg環境ではXWayland実装の制約により改善しない可能性があったが、後述の通り実機Zorin OSでは問題なく動作することを確認済み。WSL環境はあくまで開発時の参考であり、対象OS（Ubuntu/Zorin OS）実機での動作が正である。
...
- 実機Zorin OSにて、Nautilusから本アプリへのドラッグ&ドロップが正常に動作することを目視確認済み。
