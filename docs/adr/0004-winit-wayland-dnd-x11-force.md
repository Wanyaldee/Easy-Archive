...
圧縮側（ディレクトリ/ファイルをドロップ）はまだ実機で未確認（`handle_drop`の判定ロジック自体は解凍・圧縮で共通のため、動作する可能性は高いが未検証のまま記録する）。

## 既知の限界

- WSLg環境ではXWayland実装の制約により改善しない可能性があったが、実機Zorin OSでは解凍側の動作を確認済み（圧縮側はまだ実機未確認）。WSL環境はあくまで開発時の参考であり、対象OS（Ubuntu/Zorin OS）実機での動作が正である。
...
- 実機Zorin OSでの解凍側ドラッグ&ドロップは確認済み。圧縮側の実機確認は今後行う。
