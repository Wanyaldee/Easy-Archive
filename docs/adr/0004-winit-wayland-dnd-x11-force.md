...
- **WSLg環境では、X11強制後も改善しない可能性がある。** WSLgのXWayland実装が、Nautilusのようなネイティブ Waylandクライアントから X11(XWayland)ウィンドウへのドラッグ&ドロップの橋渡し（Wayland⇔X11 DNDブリッジ）を完全にはサポートしていない可能性がある。これはWSLg自体の制約であり、本プロジェクトのコードでは対処できない。開発中の実機（WSL）での最終確認ができておらず、**実機のZorin OS等での確認が必要**。
...
- 実際にドラッグ&ドロップが機能することの確認は、実機Zorin OSでの検証待ち。
