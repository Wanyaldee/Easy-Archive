...
## 追記: Nautilusスクリプトのファイル名から`.sh`拡張子を除去

ユーザーからのフィードバック「Nautilusの右クリックメニューでシェルスクリプト形式にしているが、拡張子の`.sh`がUI上のノイズになりそう」を受けて、`ここに解凍・圧縮.sh` → `ここに解凍・圧縮`(拡張子なし)に変更した。

上記「検証結果(WSL開発環境)」に記載の通り、Nautilusのscripts方式は「ファイル名がそのままメニュー表示名になる(自動整形なし)」「実行権限必須。シバン行付きスクリプトとして置く」という仕組みであり、拡張子そのものに機能的な意味はない(メニュー表示条件の出し分けにも使えない、という既存の制約記述とも整合する)。したがって拡張子を外してもスクリプトとしての認識・実行には影響しない。
